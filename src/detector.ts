...
function helper(index: number, original: string, wordsStr: string = '{ "words": [], "total_characters": 0 }'): WordList {
  let wordList = JSON.parse(wordsStr) as WordList;
  if (index >= original.length) return wordList;

  let currentOutput: WordList = wordList;
  for (let i = index + 1; i <= original.length; i++) {
    ...
    if (dictionary.check(capitalizeFirstLetter(segment)) && segment.length > MIN_LENGTH) {
      ...
      if (wordsOutput.total_characters > currentOutput.total_characters) {
        ...
        if (wordsOutput.words.length < currentOutput.words.length) currentOutput = wordsOutput;
      }
    }
  }
...
export function detectKeywords(input: string) {
  let output: WordList = { words: [], total_characters: 0 };
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (o.total_characters > output.total_characters) {
      ...
      if (o.words.length < output.words.length) output = o;
    }
  }
...
export function detectKeywordsInDomains(domains: string[]) {
  const output = [];
