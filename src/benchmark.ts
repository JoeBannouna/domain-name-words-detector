...
const input: string = fs.readFileSync(path.join('input', `question${DOMAINS_NUM}.txt`), { encoding: 'utf8' });
const domains = input.split('\n');
domains.pop();  // remove empty last line

const ans = detectKeywordsInDomains(domains);

const MODE: 'TIME' | 'WORD ACCURACY' | 'DOMAIN ACCURACY' = 'DOMAIN ACCURACY';

// @ts-ignore
if (MODE == 'TIME') console.log(performance.now());
else {
  const answerOutput: string = fs.readFileSync(path.join('input', `answer${DOMAINS_NUM}.json`), { encoding: 'utf8' });
  const correctAns: string[][] = JSON.parse(answerOutput);
  // @ts-ignore
  if (MODE == 'WORD ACCURACY') {
    ...
    ans.forEach((domain, i) => {
      let ratio = 0;
      domain.forEach(word => {
        if (correctAns[i].includes(word)) ratio++;
      });
  ...
  if (MODE == 'DOMAIN ACCURACY') {
    ...
    ans.forEach((domain, i) => {
      let ratio = 1;
      domain.forEach(word => {
        if (!correctAns[i].includes(word)) ratio = 0;
      });
      correctDomainsRatio += ratio;
    });
    correctDomainsRatio /= ans.length;
